...
// =========================================================================
// KOMPONEN KUSTOM DROPDOWN (Mirip Data Studio: Multi-Select, Search, HANYA)
// =========================================================================
const DataStudioDropdown = ({ title, options, selected, onChange }: {
  ...
  const handleToggleAll = () => {
    if (isAll) onChange([]); // Jika semua terpilih, kosongkan
    else onChange(['ALL']);  // Jika tidak, pilih semua
  };

  const handleToggleItem = (label: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (isAll) {
      // Jika tadinya 'ALL', kecualikan yang diklik
  ...
  const handleHanya = (label: string, e: React.MouseEvent) => {
    e.stopPropagation();
    onChange([label]); // Langsung atur hanya item ini yang terpilih
  };

  return (
    <div className="relative flex-1" ref={dropdownRef}>
      <div 
        className="p-2.5 text-xs border border-gray-300 rounded shadow-sm text-gray-700 bg-white flex justify-between items-center cursor-pointer font-medium"
              ...
              return (
                <div key={opt.label} className="group flex items-center gap-3 px-3 py-2 hover:bg-gray-100 cursor-pointer border-b border-gray-50 last:border-0" onClick={(e) => handleToggleItem(opt.label, e)}>
                  ...
                  <button 
                    ...
                    className="opacity-0 group-hover:opacity-100 hover:underline text-gray-600 text-[10px] font-bold px-1 rounded transition-opacity"
                  >
                    HANYA
                  </button>
...
export default function KelurahanTables({ 
  ...
  if (loading) {
    return (
      <div className="w-full h-[500px] flex items-center justify-center">
        <div className="text-[#1b75d8] font-bold text-xl animate-pulse">Memuat Data Kelurahan...</div>
      </div>
    );
  }
  ...
  // MENGUMPULKAN DATA UNTUK DROPDOWN KECAMATAN
  ...
  const kecOptions = Array.from(kecOptionsMap.entries())
    .map(([label, metric]) => ({label, metric}))
    .sort((a,b) => b.metric - a.metric); // Urutkan metrik terbanyak di atas

  // MENGUMPULKAN DATA UNTUK DROPDOWN KELURAHAN
  const availableKelData = filterKec.includes('ALL') 
    ? data 
    : data.filter(r => filterKec.includes(getVal(r, 'Kecamatan').toUpperCase()));
  ...
  const kelOptions = Array.from(kelOptionsMap.entries())
    .map(([label, metric]) => ({label, metric}))
    .sort((a,b) => b.metric - a.metric);

  // PENYARINGAN DATA TABEL
    ...
    <div className="flex gap-4 w-full">
      <div className="flex-[1.5] relative min-h-full">
        <div className="absolute inset-0 bg-[#f8faeb] p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col gap-3">
          <h2 className="font-bold text-[16px] text-black px-1 flex-none">Rekap Kelurahan</h2>
            ...
            <table className="w-full text-[11px] text-left">
              ...
              <tbody>
                {sortedDataDesc.map((row, index) => (
                  <tr key={index} className="border-b border-gray-100 even:bg-[#eef5e1] odd:bg-white hover:bg-blue-50 transition-colors">
                    <td className="px-3 py-2 text-center text-gray-500">{index + 1}.</td>
                    <td className="px-3 py-2 uppercase font-bold text-gray-800">{getVal(row, 'Kecamatan')}</td>
                    <td className="px-3 py-2 uppercase font-medium text-gray-700">{getVal(row, 'Kelurahan')}</td>
                    <td className="px-2 py-2 text-center text-gray-600">{getVal(row, 'Jumlah Kepling')}</td>
                ...
                {sortedDataDesc.length === 0 && (
                  <tr><td colSpan={8} className="text-center py-10 text-gray-400 font-bold">Data tidak ditemukan</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        {/* MENGGUNAKAN DROPDOWN KUSTOM YANG BARU DIBUAT */}
        <div className="flex gap-4">
          <DataStudioDropdown 
            ...
            onChange={(newSel) => {
              setFilterKec(newSel);
              setFilterKel(['ALL']); // Reset Kelurahan jika Kecamatan berubah
            }}
          />
          <DataStudioDropdown 
            title="Kelurahan"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-4 py-2 bg-white border-b border-gray-200">
            <h2 className="font-bold text-[14px] text-black">Top 10 Kelurahan</h2>
          </div>
            ...
            <tbody>
              {top10Data.map((row, index) => (
                <tr key={index} className="border-b border-gray-100 even:bg-[#eef5e1] odd:bg-white">
                   ...
                   <td className="px-2 py-2 text-center font-bold text-white border-l border-white/20" style={{ backgroundColor: getDynamicBgColor(getVal(row, '%')) }}>
                     {getVal(row, '%')}
                   </td>
                </tr>
              ))}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-4 py-2 bg-white border-b border-gray-200">
            <h2 className="font-bold text-[14px] text-black">Worst 10 Kelurahan</h2>
          </div>
          <table className="w-full text-[10px] text-left">
